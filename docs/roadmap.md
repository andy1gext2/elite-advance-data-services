...
## Pricing & metering ✅ (structure done, billing pending)
COGS is dominated by AI usage, unevenly: text ~$0.02/gen, image ~$0.04, **video ~$1.50–3.50/render**. So each cost tier meters separately and video overflow is paid.
- [x] **Tiers retuned for margin** (`plan_service` + migration 0012): Starter $39 (1 biz / 5 accounts / 2 seats / 150 text / 40 img / **1 video**), Professional $119 (3 / 15 / 5 / 1,000 / 250 / **8**), Agency $349 (20 / 60 / 15 / 5,000 / 1,000 / **30**), Enterprise custom (unlimited). Fields: `price_monthly`, `ai_monthly_quota`, `image_monthly_quota`, `video_monthly_quota`, `max_*`.
- [x] **Three separate meters**: text (`ai_monthly_quota`, excludes images), **image** (`image_monthly_quota`, `AiUsage.module="image"`, 402 guard, `GET …/content/image-quota`), **video** (`video_monthly_quota`, 402 guard, `GET …/content/video-quota`). Closes the old "images were unmetered" cost leak.
- [x] **Video credits** (margin protector) — `businesses.video_credits`; renders overflow past the monthly quota onto paid credits (`_consume_allowance`); `POST …/content/video-credits` grants them (billing/Stripe placeholder, admin+); the confirm dialog shows monthly + credits and a **Buy 10 renders** button when exhausted.
- [x] **Stripe billing scaffold** (`billing_service` + `app/api/billing.py`, migration 0013) — subscription checkout per tier, one-time video-credit-pack checkout, customer portal, and a **signature-verified webhook** as the source of truth (`checkout.session.completed`, `customer.subscription.updated/deleted` → set plan / add credits / downgrade). Gated on `STRIPE_SECRET_KEY`: disabled → checkout/portal 503 and the credits button **falls back to a dev grant** (so it's testable keyless). Public `GET /plans` feeds a **Billing tab** (plan cards from `price_monthly`, upgrade, buy credits, manage). Config: `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, `STRIPE_PRICE_{STARTER,PROFESSIONAL,AGENCY}`, `STRIPE_CREDITS_PRICE_ID`.
- [ ] Point live payments at it: create the Stripe products/prices + webhook endpoint, drop the IDs in `.env` (see below); optional: proration/annual prices, dunning, tax.
...
- [x] **Guaranteed product in campaign images** — each campaign post stores its `product_asset_id` (migration 0014), so generating its image **auto-grounds on that product** even if the picker isn't touched (the endpoint falls back to the post's product; the card pre-selects it). The grounded prompt now insists the product is the focal subject matching the reference, and — for apparel/footwear/accessories — shows a real person **wearing** it. Fixes product posts whose images omitted the item.
...
- [x] **Claude-directed video** — `video_service.generate_script` has **Claude turn the post's message into an 8-second Veo shot brief** (single continuous shot: subject, one motion arc, camera, lighting, mood, style; grounded on brand + product), stored on the job (`VideoJobOut.prompt`) and rendered by Veo. `TaskType.VIDEO_SCRIPT` (no module → raw provider call with a director system prompt). Falls back to the plain prompt if scripting errors. Verified live: a clothing brief → a vision that shows the garment worn by a person → a real 2.9 MB Veo clip.
